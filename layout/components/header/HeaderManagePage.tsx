...
const callback_url = "http://wordaholic.onrender.com";
...
const HeaderManagePage = () => {
  ...
  useEffect(() => {
    if (session?.user?.roles) {
      document.title = `Wordaholic | ${session.user.roles}`;
    } else {
    ...
    >
      <div className="flex justify-between flex-row mx-5 my-3">
        <div className="left flex justify-between flex-row items-center gap-4">
          ...
          />
          <h1 className="text-center text-4xl font-bold">Wordaholic</h1>
          <div className="break-line"></div>
          <p className="role-name font-bold text-lg">
            {session?.user.roles === ROLE_ADMIN && "My admin"}
          </p>
        </div>
